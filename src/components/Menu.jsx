...
const menuImages = {
  ...
  'Grilled Sea Bass': seaBassImg,
  'Couscous Royal': couscousImg,
  'Seafood Paella': paellaImg,
  'Beef Tenderloin': tenderloinImg,
  'Baklava Selection': baklavaImg,
}

const poeticLines = {
  'Lamb Chops': 'Prepared slowly over open flame, inspired by ancient traditions.',
  'Grilled Sea Bass': 'From the Mediterranean waters, served with intention.',
  'Couscous Royal': 'A family recipe, perfected through generations of love.',
  'Seafood Paella': 'Where Spanish soul meets Algerian heart.',
  'Beef Tenderloin': 'Aged with patience, served with pride.',
  'Baklava Selection': 'Sweetness that whispers of Ottoman gardens.',
}
...
const MenuCard = ({ item, delay }) => {
  ...
  const image = menuImages[item.title]
  const poetic = poeticLines[item.title] || item.origin
    ...
    >
      <div className={`relative overflow-hidden rounded-lg transition-all duration-1000 ${isRevealed ? 'opacity-100' : 'opacity-0 translate-y-12'}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
          ...
          <h3 
            className={`font-serif text-2xl md:text-3xl lg:text-4xl text-gold-400 mb-3 transition-all duration-700 ${showName ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
            style={{ 
          ...
          <div 
            className={`overflow-hidden transition-all duration-1000 ${isHovered ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}
          >
          ...
          <p 
            className={`font-sans text-sand-300/30 text-xs tracking-widest uppercase mt-4 transition-all duration-500 ${isHovered ? 'opacity-0' : 'opacity-100'} ${showDescription ? 'opacity-100' : 'opacity-0'}`}
          >
            {!isHovered && 'Touch to discover'}
          </p>
        </div>

        <div 
          className={`absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-gold-500/60 to-transparent transition-all duration-1000 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
          style={{
...
const Menu = () => {
  ...
  return (
    <section id="menu" className="relative py-24 md:py-32 lg:py-40 px-4 md:px-8 lg:px-16 bg-dark-900 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16 md:mb-20 lg:mb-28">
          <p 
            ref={subtitleRef}
            className="font-sans text-gold-500/40 text-xs tracking-[0.5em] uppercase mb-8 opacity-0"
          >
            A Private Gallery
          </p>
          <h2 
            ref={titleRef}
            className="font-serif text-4xl md:text-6xl lg:text-8xl text-gold-400 font-light opacity-0"
        ...
        <div className="flex justify-center gap-4 md:gap-8 lg:gap-12 mb-12 md:mb-16 lg:mb-20 flex-wrap">
          {categories.map(category => (
            <button
              key={category}
              onClick={() => setActiveFilter(category)}
              className={`font-sans text-xs tracking-[0.25em] uppercase transition-all duration-700 py-2 px-1 border-b-2 ${
